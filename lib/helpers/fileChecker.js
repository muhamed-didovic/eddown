...
const createLogger = downloadFolder => {
    ...
    fs.existsSync(logFile)
        ? logger.warn(`[warn] File ${logFile} already exists`)
        : logger.debug(`File ${logFile} created`);
...
const findNotExistingVideo = (videos, downloadFolder) => {
    ...
    for (let video of videos) {
        ...
        if (fs.existsSync(filename) && isCompletelyDownloaded(name, downloadFolder)) {
            logger.debug(`File "${name}" already exists`);
            i++;
        } else {
            break;
        }
...
const removeDownloadedLessons = async (course, opts, pages, lessonsByCheerio) => {
    ...
    if (!await fs.exists(logFile)) {
        logger.warn('[warn] Downloads file is not found!!!');
    ...
    logger.debug(`Length of Downloaded lessons: ${downloadsFile.length} and length of whole course: ${lessonsByCheerio.length} and their difference is: ${lessonsByCheerio.length - downloadsFile.length}`);

    const links = lessonsByCheerio.filter((lesson, index) => {
        /*logger.debug('comapre',
    ...
    logger.debug('Remaining lessons left:', links.length);
    return links
};
