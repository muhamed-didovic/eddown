                       ...
                       /* const propertyJsHandles = await Promise.all(images.map(handle => {
                        ...
                        counters = await Promise
                            .mapSeries(images, async (image, i) => {
                                ...
                                // const isVisible = await image.waitForSelector('visible');
                                ...
                                if (bound) {
                                    ...
                                    // await image.screenshot({ path: `image_${src}.png` });
                                    // return {
                                    //     'ima': i,
                                    //     src: await image.evaluate(el => el.src)
                                    // }
                            ...
                            .then(async counters => {
                                // console.log('1counters', counters);
                                counters = counters.filter(Boolean)//Number.isInteger
                                // console.log('2counters', counters);
