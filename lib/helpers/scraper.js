                        ...
                        counters = await Promise
                            .mapSeries(images, async (image, i) => {
                                ...
                                if (fs.existsSync(path.join(imagesFolder, `screenshot_${i}.png`))) {
                                    logger.warn('[warn] image is downloaded already:', path.join(imagesFolder, `screenshot_${i}.png`));
                                    return;
                                }
                                ...
                                if (bound && bound.width > 0) {
                                    ...
                                    if (width === 0) {
                                        logger.error('Screenshot has a width of 0!');
                                        return false
                                    } else {
                                        logger.debug(`Screenshot has dimensions ${width}x${height}.`);
                                    }
                        ...
                        objsCounter = await Promise
                            .mapSeries(objs, async (svgElement, i) => {
                                ...
                                if (fs.existsSync(path.join(imagesFolder, `obj-${i}.png`))) {
                                    logger.warn('[warn] obj if downloaded', path.join(imagesFolder, `obj-${i}.png`));
                                    return;
                                }
