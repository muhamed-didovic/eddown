...
module.exports = class Crawler {
        ...
        return await this.withBrowser(async (browser) => {
            return await this.withPage(browser)(async (page) => {
                ...
                .mapSeries(courses, async course => {
                    ...
                    // let lessons = [
                    //     {
                    //         "series": "Database Design Fundamentals for Software Engineers",
                    //         "downPath": "Database Design Fundamentals for Software Engineers",
                    //         "url": "https://www.educative.io/courses/database-design-fundamentals/myBA0zVyl1E",
                    //         "title": "Common Relational Database Operations",
                    //         "position": 34
                    //     }
                    // ]
                    /*let lessons = [
                ...
                const result1 = await Promise.all([
                    ...
                    (async () => {
                        try {
                            ...
                            // await page.evaluate((selector) => {
                            //     const buttons = document.querySelectorAll('button[data-testid="canvas-animation-plus"]');
                            //     buttons.forEach(async (button) => {
                            //         await new Promise(resolve => setTimeout(resolve, 2e3));
                            //         if (button.offsetParent !== null) {
                            //             button.click();
                            ...
                            //     })
                            // }, 'button[data-testid="canvas-animation-plus"]');
                            /*await page.$$eval('button[data-testid="canvas-animation-plus"]', elHandles => elHandles.forEach(async (el, i) => {
                                console.log('el', el, i);
                                const bound = await el.boundingBox() !== null;
                                if (bound) {
                                    el.click()
                                }
                            ...
                            // console.log('error Slides not found:', e);
                            return false;
                        }
                    })(),
                    (async () => {
    ...
    async getLessonsByCheerio(course) {
        ...
        .map((i, elem) => {
            ...
            //#selenium-collection-cat-link_COLLECTION_ASSESSMENT_7 > .body-small.text-gray-A400.dark\:text-gray-400
            return ({
                url: `https://www.educative.io${$(elem).attr('href')}`,
                title: $(elem).find('.body-small.text-gray-A400').text() //span
                .replaceAll('\\W+', '')
                //.replace('\\nStart\\n', '')
                .replace(/[.,]/g, '') //remove dots and commas
                .replace(/(\r\n|\n|\r)/gm, '')
