...
module.exports = class Crawler {
    ...
    async scrape(browser, page, course, position, lesson, opts) {
        ...
        await this.retry(async () => {
            try {
                ...
                logger.debug('[IMPORTANT] result of scraping of a page: ', result1, lesson.title);
    ...
    async solveQuiz(browser, page, course, position, lesson, opts, counter) {
        try {
            ...
            logger.debug('quizes found on page:', elHandleArray.length);
            await Promise
            .mapSeries(elHandleArray, async (el, key) => {
                if (counter != 1) {
    ...
    fileIsDownloaded(course, opts, position, lesson) {
        ...
        logger.debug('isDownloaded', isDownloaded, lesson.title);
        return isDownloaded;
    }
