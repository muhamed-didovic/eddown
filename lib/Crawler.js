...
module.exports = class Crawler {
    ...
    async getCourseForDownload(page, link, { all }) {
        ...
        //let links = require(path.resolve(process.cwd(), 'json/search-courses.json'));
        ...
        console.log('Total number of courses found:', links.length);
        //remove courses that are downloaded already
        if (await fs.exists(path.resolve(process.cwd(), 'json/downloaded-courses.json'))) {
            const downloadedCourses = await require(path.resolve(process.cwd(), 'json/downloaded-courses.json'))
                ...
                const result1 = await Promise.all([
                    (async () => {
                        ...
                        try {
                            ...
                            //todo: check if there is better way to click on mulitple buttons
                    ...
                    (async () => {
                        ...
                        try {
                            ...
                            const dest = path.join(process.cwd(), opts.dir, series, 'code', `${String(index + 1).padStart(2, '0')}-${lesson.title}`)
                            fs.ensureDir(dest)
                            //download file
    ...
    async getLessons(browser, page, course, ms, opts) {
        ...
        await page.waitForSelector('h1.text-4xl', { timeout: 22e3 })
        ...
        if (!lessons.length) {
            lessons = await page.evaluate(() => {
                const links = Array.from(document.body.querySelectorAll("h2 .flex.flex-col.w-full a"), a => {
                    return ({
                        url  : a.href,
                        title: a.innerText
                            .replaceAll('\\W+', '')
